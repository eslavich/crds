        ...
        if exptime < SM4:
            if numcols > ACS_HALF_CHIP_COLS:
                if header["CCDAMP"] in ["A", "D"]:
                    log.verbose(
                        "acs_bias_file_selection: exposure is pre-SM4, converting amp A or D "
                        + "to AD for NUMCOLS = "
                        + header["NUMCOLS"]
                    )
                    header["CCDAMP"] = "AD"
                    ...
                    log.verbose(
                        "acs_bias_file_selection: exposure is pre-SM4, converting amp B or C "
                        + "to BC for NUMCOLS = "
                        + header["NUMCOLS"]
                    )
                    header["CCDAMP"] = "BC"
    if header["DETECTOR"] == "WFC" and header["XCORNER"] == "0.0" and header["YCORNER"] == "0.0":
...
def rmap_update_headers_acs_biasfile_v1(rmap, header_in):
    ...
    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="C", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="0.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])

    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="D", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="0.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])

    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="C|D", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="0.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])

    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="A", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="20.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])

    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="B", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="20.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])

    if header_matches(
        header, dict(DETECTOR="HRC", CCDAMP="A|B", NUMROWS="1044.0", NUMCOLS="1062.0", LTV1="19.0", LTV2="20.0")
    ):
        return dont_care(header, ["NUMROWS", "NUMCOLS", "LTV1", "LTV2"])
    ...
    for i, name in enumerate(parkeys):
        if name in matches:
            if utils.condition_value(matches[name]) != utils.condition_value(key[i]):
                log.verbose(
                    "Exiting on", repr(name), utils.condition_value(matches[name]), utils.condition_value(key[i])
                )
                return False
