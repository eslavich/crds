...
def wfc3_biasfile_filter(kmap):
    ...
    """
    header_additions = {"hooks": {"precondition_header": "precondition_header_wfc3_biasfile_v1"}}
...
def precondition_header_wfc3_biasfile_v1(rmap, header_in):
    ...
    if header["SUBARRAY"] == "T" and "SUB" not in header["APERTURE"]:
        header["APERTURE"] = "N/A"
        log.verbose(
            "Mutated APERTURE to ", repr(header["APERTURE"]), "based on SUBARRAY='T' and 'SUB' not in APERTURE."
        )
    return header
