...
class MappingDifferencer(Differencer):
    ...
    def __init__(self, *args, **keys):
        ...
        assert rmap.is_mapping(self.old_file), "File " + repr(self.old_file) + " is not a CRDS mapping."
        assert rmap.is_mapping(self.new_file), "File " + repr(self.new_file) + " is not a CRDS mapping."
                ...
                if "replaced" in diff[-1]:
                    ...
                    if old and new:
                        ...
                        diff.difference(
                            ...
                            old,
                            new,
                            primitive_diffs=self.primitive_diffs,
    ...
    def mapping_diffs(self):
        ...
        differences = old_map.difference(
            new_map, include_header_diffs=self.include_header_diffs, recurse_added_deleted=self.recurse_added_deleted
        )
        return differences
        ...
        for diff in diffs:
            for step in diff:
                ...
                # New rmaps imply reprocessing the entire type.
                    ...
                    isinstance(diff[-1], str)
                    and diff[-1].startswith(("added", "deleted"))
                    and diff[-1].endswith(".rmap'")
                ):
...
class FitsDifferencer(Differencer):
    ...
    def __init__(self, *args, **keys):
        ...
        assert self.old_file.endswith(".fits"), "File " + repr(self.old_file) + " is not a FITS file."
        assert self.new_file.endswith(".fits"), "File " + repr(self.new_file) + " is not a FITS file."
...
class TextDifferencer(Differencer):
    ...
    def diff(self):
        ...
        loc_old_file = self.locate_file1(self.old_file)
        loc_new_file = self.locate_file2(self.new_file)
        status, out_err = pysh.status_out_err(
            "diff -b -c ${loc_old_file} ${loc_new_file}", raise_on_error=False
        )  # secure
...
class AsdfDifferencer(Differencer):
    ...
    def diff(self):
        ...
        loc_new_file = self.locate_file2(self.new_file)
        status, out_err = pysh.status_out_err(
            "asdftool diff ${loc_old_file} ${loc_new_file}", raise_on_error=False
        )  # secure
...
DEFAULT_EXCLUDED_PARAMETERS = (
    ...
    "META_OBSERVATION_DATE",
    "META_OBSERVATION_TIME",
    "DIFFERENCE",
    "INSTRUME",
    "REFTYPE",
)
...
def mapping_check_references(mapping, derived_from):
    ...
    if old_refs - new_refs:
        log.warning(
            "Deleted references for",
            repr(derived_from.filename),
            "and",
            repr(mapping.filename),
            "=",
...
Mutually Exclusive Modes
    ...
    def main(self):
        ...
        if self.args.sync_files:
            assert not (
                self.args.cache1 or self.args.cache2
        ...
        assert (self.args.cache1 and self.args.cache2) or (
            not self.args.cache1 and not self.args.cache2
    ...
    def print_affected_modes(self):
        ...
        assert rmap.is_mapping(self.old_file) and rmap.is_mapping(
            self.new_file
