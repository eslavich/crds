...
def _get_pipelines(exp_type, cal_ver=None, context=None):
    ...
    """
    with log.augment_exception("Failed determining required pipeline .cfgs for", "EXP_TYPE", srepr(exp_type)):
        config_manager = _get_config_manager(context, cal_ver)
...
def reftype_to_pipelines(reftype, cal_ver=None, context=None):
    ...
    exp_type.
    """
    with log.augment_exception("Failed determining required pipeline .cfgs for", "REFTYPE", srepr(reftype)):
        config_manager = _get_config_manager(context, cal_ver)
...
def _get_config_refpath(context, cal_ver):
    ...
    try:  # Use a normal try/except because exceptions are expected.
        header = {"META.INSTRUMENT.NAME": "SYSTEM", "META.CALIBRATION_SOFTWARE_VERSION": cal_ver}
    ...
    log.verbose(
        "Using", srepr(os.path.basename(refpath)), "to determine applicable default reftypes for", srepr(cal_ver)
    )
    return refpath
...
class CrdsCfgManager:
    ...
    def exptype_to_reftypes(self, exp_type):
        ...
        log.verbose(
            "Applicable reftypes for",
            srepr(exp_type),
            "determined by",
            srepr(os.path.basename(self._refpath)),
            "are",
            srepr(reftypes),
        )
        return reftypes
...
def invert_list_mapping(mapping):
    ...
    return {key: list(sorted(values)) for (key, values) in inverted.items()}
...
def _get_reftypes(exp_type, cal_ver=None, context=None):
    ...
    """
    with log.warn_on_exception("Failed determining required reftypes from", "EXP_TYPE", srepr(exp_type)):
        config_manager = _get_config_manager(context, cal_ver)
