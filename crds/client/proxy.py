...
class CheckingProxy:
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(url='%s', version='%s')" % (self.__service_url, self.__version)
...
class ServiceCallBinding:
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(url='%s', method='%s')" % (self.__service_url, self.__service_name)
    ...
    def _call(self, *args, **kwargs):
        ...
        if "serverless" in url or "server-less" in url:
            raise exceptions.ServiceError(
                "Configured for server-less mode.  Skipping JSON RPC " + repr(self.__service_name)
            )
...
def crds_encode(obj):
    ...
    """
    return dict(crds_encoded="1.0", crds_payload=json.dumps(obj).encode("zlib").encode("base64"))
