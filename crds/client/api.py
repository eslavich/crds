    ...
    for filetype, refname in bestrefs.items():
        if "NOT FOUND" in refname:
            if refname.upper() == "NOT FOUND N/A":
                ...
                raise CrdsLookupError("Error determining best reference for", srepr(filetype), "=", repr(exc_str))
    return bestrefs
...
def get_server_info():
    ...
    try:
        ...
        raise CrdsNetworkError("network connection failed:", srepr(get_crds_server()), ":", srepr(exc)) from exc
...
def get_default_observatory():
    ...
    """
    return (
        os.environ.get("CRDS_OBSERVATORY", None)
        or observatory_from_string(get_crds_server())
        or get_server_observatory()
        or "jwst"
    )
...
class FileCacher:
    ...
    def download_files(self, downloads, localpaths):
        ...
        if config.writable_cache_or_verbose(
            "Readonly cache, skipping download of (first 5):", repr(downloads[:5]), verbosity=70
        ):
            bytes_so_far = 0
    ...
    def download(self, name, localpath):
        ...
        try:
            ...
            raise CrdsDownloadError(
                ...
                srepr(name),
                "at CRDS server",
                srepr(get_crds_server()),
                "with mode",
                srepr(config.get_download_mode()),
                ":",
                str(exc),
            ) from exc
    ...
    def plugin_download(self, filename, localpath):
        ...
        if status != 0:
            if status == 2:
                ...
                raise CrdsDownloadError(
                    "Plugin download fail status =", repr(status), "with command:", srepr(plugin_cmd)
                )
    ...
    def get_data_http(self, filename):
        ...
        try:
            ...
            while data:
                ...
                log.verbose(
                    "Transferred HTTP", repr(url), bytes_so_far, "/", file_size, "bytes at", status[1], verbosity=20
                )
                yield data
                data = infile.read(config.CRDS_DATA_CHUNK_SIZE)
        except Exception as exc:
            raise CrdsDownloadError(
                "Failed downloading", srepr(filename), "from url", srepr(url), ":", str(exc)
            ) from exc
        finally:
            try:
                infile.close()
    ...
    def verify_file(self, filename, localpath):
        ...
        if original_length != local_length and config.get_length_flag():
            raise CrdsDownloadError("downloaded file size", local_length, "does not match server size", original_length)
        if not config.get_checksum_flag():
            ...
            if original_sha1sum != local_sha1sum:
                raise CrdsDownloadError(
                    ...
                    "sha1sum",
                    srepr(local_sha1sum),
                    "does not match server sha1sum",
                    srepr(original_sha1sum),
                )
        else:
            ...
            if "NOT FOUND" in refname:
                if "n/a" in refname.lower():
                    ...
                    last_error = CrdsLookupError(
                        "Error determining best reference for", srepr(filetype), " = ", str(refname)[len("NOT FOUND") :]
                    )
...
def _squash_unicode_in_bestrefs(bestrefs, localrefs):
    ...
    for filetype, refname in bestrefs.items():
        if isinstance(refname, tuple):
            ...
            raise CrdsLookupError("Unhandled bestrefs return value type for", srepr(filetype))
    return refs
