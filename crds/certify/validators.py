...
class Validator:
    ...
    def __init__(self, info):
        ...
        if not (self.info.presence in generic_tpn.TpnInfo.presences or self.conditionally_required):
    ...
    def handle_missing(self, header):
        ...
        if presence in ["R", "P", True]:
            ...
            raise TpnDefinitionError("Unexpected validator 'presence' value:", repr(self.info.presence))
        return "UNDEFINED"
    ...
    def is_applicable(self, header):
        ...
        if self._presence_condition_code:
            try:
                ...
                log.verbose(
                    "Validator", self.info, "is", "applicable." if presence else "not applicable.", verbosity=70
                )
                if not presence:
                    return False
...
class KeywordValidator(Validator):
    ...
    def _check_value(self, filename, value):
        ...
        if self._match_value(value):
            ...
            raise ValueError("Value " + str(log.PP(value)) + " is not one of " + str(log.PP(self._values)))
        ...
        try:
            ...
            for possible in self._values:
                if np.allclose(value, possible, self.epsilon):
                    ...
                    return
            msg = " ".join(
                [self.type_name, repr(value), "is not within +-", repr(self.epsilon), "of any of", repr(self._values)]
            )
...
class PedigreeBaseValidator(KeywordValidator):
    ...
    def _inflight_exc(self, clarification=""):
        ...
        """
        return ValueError("INFLIGHT format should be " + repr(self._dated_format) + " : " + clarification)
        ...
        with log.augment_exception("Invalid value:", repr(value)):
            ...
            if len(values) == 1:
                if pedigree not in self._values_wo_date:
                    raise ValueError("Value " + repr(pedigree) + " cannot be used as a simple value.")
                else:
                    return pedigree
            elif len(values) == 3:
                if pedigree not in self._values_w_date:
                    raise ValueError("Value " + repr(pedigree) + " cannot be specified with <start> and <stop> dates.")
            else:
            ...
            for char in start + stop:
                if char in ["T", " ", ":"]:
                    raise self._inflight_exc("Time should not appear in INFLIGHT dates.")
...
class ExpressionValidator(Validator):
    ...
    def check_header(self, filename, header):
        ...
        """
        log.verbose(
            "File=" + repr(os.path.basename(filename)), "Checking", repr(self.name), "condition", str(self._expr)
        )
        for keyword in expr_identifiers(self._expr):
            if header.get(keyword, "UNDEFINED") == "UNDEFINED":
                log.verbose_warning("Keyword or Array", repr(keyword), "is 'UNDEFINED'. Skipping ", repr(self._expr))
        ...
        for (i, image) in enumerate(images_data):
            if abs(image.sum() - 1.0) > 1.0e-6:
                raise BadKernelSumError(
                    "Kernel sum", image.sum(), "is not 1+-1e-6 for kernel #" + str(i), ":", repr(image)
                )
