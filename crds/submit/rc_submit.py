...
BASE_URLS = {
    "dev": {"hst": "https://hst-crds-dev.stsci.edu/", "jwst": "https://jwst-crds-dev.stsci.edu/"},
    "test": {"hst": "https://hst-crds-test.stsci.edu/", "jwst": "https://jwst-crds-test.stsci.edu/"},
    "ops": {"hst": "https://hst-crds.stsci.edu/", "jwst": "https://jwst-crds.stsci.edu/"},  # 'ops' not in URL
    "bit": {"hst": "https://hst-crds-bit.stsci.edu/", "jwst": "https://jwst-crds-bit.stsci.edu/"},
    "cit": {"hst": "https://hst-crds-cit.stsci.edu/", "jwst": "https://jwst-crds-cit.stsci.edu/"},
}

URL_DESCRIPTION = "submission_form/redcat_description.yml"

NULL_FIELDTYPES = {"BooleanField": bool, "CharField": str, "TypedChoiceField": str}
...
class RedCatSubmissionScript(RedCatApiScript):
    ...
    def add_args(self):
        ...
        self.add_argument("--redcat-parameters", type=str, help="Path to YAML file defining extra ReDCaT parameters.")
...
class Submission(object):
    ...
    def __init__(self, observatory, string, *args, **kwargs):
        ...
        for key in self._form_description:
            self._form_description[key].pop("key")
    ...
    def __repr__(self):
        return "<Submission Object {}-{}>:\nFields:  {}\nFiles:  {}".format(
            self.observatory, self.string, self._fields.__repr__(), list(self.files)
        )
    ...
    def __setitem__(self, key, value):
        ...
        if (
            ...
            and ("choices" in self._form_description[key])
            and not (self._form_description[key]["type"] == "BooleanField")
        ):
            if value:
                value = "Yes"
            else:
        ...
        if ("choices" in self._form_description[key]) and (self._form_description[key]["type"] != "BooleanField"):
            matches = [x for x in self._form_description[key]["choices"] if x.lower() == value.lower()]
            if len(matches) != 1:
                raise ValueError(
                    "'{}' must be a valid choice: {{{}}}".format(key, ", ".join(self._form_description[key]["choices"]))
                )
    ...
    def help(self):
        ...
        for key, field in self._form_description.items():
            print(
                key,
                " (",
                NULL_FIELDTYPES[self._form_description[key]["type"]].__name__,
    ...
    def validate(self):
        ...
        if len(self.files) == 0:
            raise NoFilesSelected(
                "No files have been added to submission.  " "Use the `submission_obj.add_file()` method."
            )
    ...
    def submit(self):
        ...
        return SubmissionResult(
            error_count=script._error_count, warning_count=script._warning_count, ready_url=script._ready_url
        )
