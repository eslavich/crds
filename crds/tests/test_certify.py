...
class TestCertify(test_config.CRDSTestCase):
    ...
    def test_missing_column_validator(self):
        ...
        checker = certify.validator(info)
        assert_raises(
            certify.MissingKeywordError,
            checker.check,
            self.data("v8q14451j_idc.fits"),
    ...
    def test_tpn_pedigree_missing(self):
        ...
        checker = certify.validator(info)
        assert_raises(certify.MissingKeywordError, checker.check, "test.fits", {"DETECTOR": "This is a test"})
    ...
    def test_tpn_pedigree_bad_datetime_format_3(self):
        info = certify.TpnInfo("PEDIGREE", "H", "C", "R", ["&PEDIGREE"])
        checker = certify.validator(info)
        assert_raises(ValueError, checker.check, "test.fits", {"PEDIGREE": "INFLIGHT 2017-01-01T00:00:00 2017-01-02"})
    ...
    def test_tpn_jwstpedigree_ground_dates(self):
        info = certify.TpnInfo("PEDIGREE", "H", "C", "R", ["&JWSTPEDIGREE"])
        checker = certify.validator(info)
        assert_raises(ValueError, checker.check, "test.fits", {"PEDIGREE": "GROUND 2018-01-01 2018-01-25"})
    ...
    def test_tpn_jwstpedigree_missing_format_3(self):
        info = certify.TpnInfo("PEDIGREE", "H", "C", "R", ["&JWSTPEDIGREE"])
        checker = certify.validator(info)
        assert_raises(certify.MissingKeywordError, checker.check, "test.fits", {})

    def test_tpn_jwstpedigree_no_model_3(self):
        info = certify.TpnInfo("PEDIGREE", "H", "C", "R", ["&JWSTPEDIGREE"])
        checker = certify.validator(info)
        assert_raises(ValueError, checker.check, "test.fits", {"PEDIGREE": "MODEL"})
    ...
    def test_table_mode_checks_identical(self):
        self.certify_files(
            [self.data("v8q14451j_idc.fits")], observatory="hst", context="hst.pmap", compare_old_reference=True
        )

    def test_table_mode_checks_missing_modes(self):
        self.certify_files(
            [self.data("v8q1445xx_idc.fits")], observatory="hst", context="hst.pmap", compare_old_reference=True
        )
    ...
    def test_UnknownCertifier_missing(self):
        ...
        assert_raises(
            ...
            self.data("non-existent-file.txt"),
            observatory="jwst",
            context="jwst.pmap",
        )
    ...
    def test_FitsCertify_bad_value(self):
        assert_raises(
            ValueError,
            certify.certify_file,
            self.data("s7g1700gm_dead_broken.fits"),
            observatory="hst",
            context="hst.pmap",
        )
    ...
    def test_certify_kernel_unity_validator_good(self):
        header = {
            "SCI_ARRAY": utils.Struct(
                {
                    "COLUMN_NAMES": None,
                    "DATA": np.array(
                        [[0.0, 0.0276, 0.0], [0.0316, 0.88160002, 0.0316], [0.0, 0.0276, 0.0]], dtype="float32"
                    ),
                    "DATA_TYPE": "float32",
                    "EXTENSION": 1,
                    "KIND": "IMAGE",
            ...
            "SCI_ARRAY": utils.Struct(
                {
                    "COLUMN_NAMES": None,
                    "DATA": np.array(
                        [[0.0, 0.0276, 0.0], [0.0316, 0.88160002 + 1e-6, 0.0316], [0.0, 0.0276, 0.0]], dtype="float32"
                    ),
                    "DATA_TYPE": "float32",
                    "EXTENSION": 1,
                    "KIND": "IMAGE",
