...
def get_hdulist(fits_reference):
    ...
    if isinstance(fits_reference, str):
        try:
            ...
            raise IOError(
                "error opening file (%s): %s: %s" % (fits_reference, exc.__class__.__name__, exc.args[0])
            ) from exc
    return result
...
def hdus_consistent(a_hdulist, b_hdulist):
    ...
    if len(a_hdulist) != len(b_hdulist):
        raise RuntimeError("HDUs have different lengths: FITS A = %d, FITS B = %d" % (len(a_hdulist), len(b_hdulist)))

    # Loop through the HDUs and check types
    # If one is different, then abort.
    for index in range(len(a_hdulist)):
        if a_hdulist[index].header.get("XTENSION") != b_hdulist[index].header.get("XTENSION"):
            raise RuntimeError(
                "Extension %d different: FITS A = %s, FITS B = %s"
                % (index, a_hdulist[index].header.get("XTENSION"), b_hdulist[index].header.get("XTENSION"))
            )
...
class RowDiff:
    ...
    def __init__(self, a_fits, b_fits, fields=[], ignore_fields=[], mode_fields=[]):
        ...
        if self.fields and self.ignore_fields:
            raise RuntimeError("Both fields and ignore_fields" + " cannot be specified.")

        # Get the FITS HDU's.
        with get_hdulist(a_fits) as self.a_hdulist, get_hdulist(b_fits) as self.b_hdulist:
            ...
            for hdu_index in range(len(self.a_hdulist)):
                if isinstance(self.a_hdulist[hdu_index], _TableLikeHDU):
                    self.diffs.append(
                        (hdu_index, self.diff(self.a_hdulist[hdu_index].data, self.b_hdulist[hdu_index].data))
                    )
    ...
    def modediff(self, a_fitstable, b_fitstable):
        ...
        data_diff = TableDataDiff(a_fitstable, b_fitstable, ignore_fields=self.ignore_fields, numdiffs=1)
        ...
        if self.fields:
            fields_common = list_intersection(self.fields, data_diff.common_column_names)
        else:
            ...
            fields_common = list_intersection(
                a_fitstable.columns.names, data_diff.common_column_names, lambda element: element.lower()
            )
        ...
        for field in mode_field_names:
            values_possible.append(sorted(list(set(list(a_table_modes[field]) + list(b_table_modes[field])))))
        ...
        if data_diff.diff_rows or data_diff.diff_values:
            ...
            if len(matching_blocks) > 1:
                ...
                fields_all = list_intersection(a_table.colnames, mode_field_names + fields_common)
                ...
                a_table_values = Table(names=a_table_common_fields.colnames, dtype=dtypes)
                b_table_values = Table(names=a_table_common_fields.colnames, dtype=dtypes)
                ...
                # only those rows.
                if isinstance(self.mode_fields, dict):
                    selected_rows = np.array([True for index in range(len(a_table_values))])
                    for (field, values) in dict.items(self.mode_fields):
                        if values:
                            selected_rows = np.logical_and(selected_rows, selected(a_table_values[field], values))
    ...
    def rowdiff(self, a_fitstable, b_fitstable):
        ...
        result = None
        if sm.ratio() < 0.99:
            unified_diff = difflib.unified_diff(a_string, b_string, "Table A", "Table B")
        ...
        if len(self.diffs) > 0:
            ...
            if self.mode_fields:
                ...
                for (diff_current, diff) in self.diffs:
                    ...
                    (result_modes_vs_a, result_modes_vs_b, result_a_vs_b, common_mode_diffs) = diff
...
class RowDiffScript(cmdline.Script):
    ...
    def add_args(self):
        ...
        self.add_argument("--mode-fields", help="List of fields to do a mode compare", type=str)
    ...
    def main(self):
        ...
        print(RowDiff(tableA_path, tableB_path, fields=fields, ignore_fields=ignore_fields, mode_fields=mode_fields))
