...
than errors as the default.
    ...
    def complex_init(self):
        ...
        exclusive_source_modes = [
            self.args.files,
            self.args.datasets,
            self.args.instruments,
            self.args.diffs_only,
            self.args.all_instruments,
        ]
        source_modes = len(exclusive_source_modes) - exclusive_source_modes.count(None)
        using_pickles = int(bool(self.args.load_pickles))
        assert (
            source_modes <= 1 and (source_modes + using_pickles) >= 1
        ...
        if self.args.diffs_only:
            ...
            differ = diff.MappingDifferencer(
                self.observatory, self.old_context, self.new_context, include_header_diffs=True, hide_boring_diffs=True
            )
            self.affected_instruments = differ.get_affected()
            log.info(
                "Mapping differences from",
                repr(self.old_context),
                "-->",
                repr(self.new_context),
                "affect:\n",
            ...
            if not self.instruments:
                ...
                return False
            if self.args.datasets_since == "auto" and (differ.header_modified() or differ.files_deleted()):
    ...
    def auto_datasets_since(self):
        ...
        log.info(
            "Possibly affected --datasets-since dates determined by",
            repr(self.old_context),
            "-->",
            repr(self.new_context),
            "are:\n",
    ...
    def add_args(self):
        ...
        self.add_argument(
            ...
            "--skip-types",
            nargs="+",
            metavar="SKIPPED_REFERENCE_TYPES",
            default=(),
    ...
    def setup_contexts(self):
        ...
        if self.args.new_context is None:
            log.verbose(
                "Using default new context",
                repr(self.default_context),
                "for computing updated best references.",
                verbosity=25,
            )
            new_context = self.default_context
        else:
            log.verbose(
                "Using explicit new context",
                repr(self.args.new_context),
                "for computing updated best references.",
                verbosity=25,
            )
    ...
    def init_headers(self, context, datasets_since):
        ...
        if self.args.files:
            ...
            the_headers = headers.DatasetHeaderGenerator(
                context, [dset.upper() for dset in self.args.datasets], datasets_since
            )
            ...
            log.error(
                "Invalid header source configuration.   "
    ...
    def init_comparison(self, datasets_since):
        ...
        assert not (
            self.args.old_context and self.args.compare_source_bestrefs
        ), "Cannot specify both --old-context and --compare-source-bestrefs."
        compare_prior = (
            self.args.old_context
            or self.args.compare_source_bestrefs
            or self.args.print_affected
            or self.args.print_affected_details
        )
    ...
    def main(self):
        ...
        if self.args.eliminate_duplicate_cases:
            log.warning(
                "Running in --eliminate-duplicate-cases mode;  even successful bestrefs are categorized as errors for analysis."
            )
        log.verbose(self.get_stat("datasets"), "sources processed", verbosity=5)
    ...
    def get_bestrefs(self, instrument, dataset, context, header):
        ...
        with log.augment_exception(
            "Failed determining reference types for", repr(dataset), "with respect to", (instrument, context, header)
        ):
            reftypes = self.determine_reftypes(instrument, dataset, context, header)
            if reftypes is None:
                return {}
        with log.augment_exception(
            "Failed computing bestrefs for data", repr(dataset), "with respect to", repr(context)
        ):
    ...
    def _compare_bestrefs(self, instrument, dataset, oldrefs, newrefs):
        ...
        for filekind in filekinds:
            ...
            if not (
                new_ok or self.args.update_pickle
            ):  # ERROR's in new cannot update,  except during regression capture
                kill_list.append(update)
                continue
            ...
            if new != old:
                if self.args.differences_are_errors:  # regression mode vs. reprocessing
                    ...
                    log.info(
                        self.format_prefix(dataset, instrument, filekind),
                        "New best reference:",
                        sreprlow(old),
                        "-->",
                        sreprlow(new),
                ...
                self.verbose_with_prefix(
                    dataset, instrument, filekind, "Lookup MATCHES:", sreprlow(old), self.no_update, verbosity=30
                )
        return updates, kill_list
    ...
    def handle_na_and_not_found(self, name, bestrefs, dataset, instrument, filekind):
        ...
        if ref.startswith("NOT FOUND N/A"):
            ...
            if self.args.undefined_differences_matter:  # track these when N/A is being scrutinized, regardless of diff.
                self.log_and_track_error(dataset, instrument, filekind, name, "No match found:", repr(ref_org))
                ref_ok = False
            else:
                self.verbose_with_prefix(
                    dataset, instrument, filekind, name, "No match found:", repr(ref_org), " => 'N/A'."
                )
                ref = "N/A"
    ...
    def optimize_tables(self, dataset, updates):
        ...
        for update in sorted(updates):
            ...
            if not table_effects.is_reprocessing_required(
                dataset, new_header, self.old_context, self.new_context, update
            ):
                updates.remove(update)  # reprocessing not required, ignore update.
                log.verbose(
                    "Removing table update for",
                    update.instrument,
                    update.filekind,
                    dataset,
                    "no effective change from reference",
                    repr(update.old_reference),
                    "-->",
                    repr(update.new_reference),
                    verbosity=25,
                )
        return updates
    ...
    def unkilled_updates(self):
        ...
        return {
            dataset: updates for (dataset, updates) in self.updates.items() if updates and dataset not in self.kill_list
        }
    ...
    def print_affected(self):
        ...
        """
        affected_products = {self.dataset_to_product_id(dataset) for dataset in self.unkilled_updates}
    ...
    def eliminate_duplicate_cases(self):
        ...
        for dataset in self.updates:
            ...
            self.log_and_track_error(dataset, "any", "any", "COVERAGE:", self.updates_repr(updates))
    ...
    def sync_references(self):
        ...
        synced_references = {
            ...
            for dataset in self.updates
            for tup in self.updates[dataset]
            if tup.new_reference not in ["N/A"]
        }
...
def assign_bestrefs(filepaths, context=None, reftypes=(), sync_references=False, verbosity=-1):
