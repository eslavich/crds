    ...
    def __init__(self, *command, **keys):
        ...
        if self._capture_output:
            if self._independent_error:
                self._popen = Popen(command, shell=self._use_shell, stdin=PIPE, stdout=PIPE, stderr=PIPE, **keys)
            else:
                self._popen = Popen(command, shell=self._use_shell, stdin=PIPE, stdout=PIPE, stderr=STDOUT, **keys)
        else:
...
def _context(backup):
    ...
    context.update(args)
    context = {key: shlex.quote(val) for key, val in context.items() if isinstance(val, str)}
    return context
