...
def get_lock_class():
    ...
    classes = {"multiprocessing": CrdsMultiprocessingLock, "filelock": CrdsFileLock, "lockfile": CrdsLockFile}
...
writes."""
    ...
    if not config.USE_LOCKING.get():
        _explain_once(
            "CRDS_USE_LOCKING = False. Cannot support downloading CRDS files while multiprocessing.", log.verbose
        )
        ...
        try:
            ...
            _explain_once("Failed creating CRDS cache lock: " + str(exc), log.warning)
    return lock
