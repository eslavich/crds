...
DEFAULT_SUBSTITUTIONS = {"META.SUBARRAY.NAME": {"GENERIC": "N/A"}, "SUBARRAY": {"GENERIC": "N/A"}}
...
class Selector:
    ...
    def __init__(self, parameters, selections=None, rmap_header=None, merge_selections=None):
        assert isinstance(parameters, (list, tuple)), "parameters should be a list or tuple of header keys"
        ...
        if selections is not None:
            assert isinstance(selections, dict), "selections should be a dictionary { key: choice, ... }."
            ...
            assert isinstance(merge_selections, list), "merge_selections should be a sorted item list,  not: " + repr(
                merge_selections
            )
    ...
    def _substitute_tuple_value(self, match, keyword):
        ...
        if old_parvalue in self._substitutions[keyword]:
            ...
            log.verbose(
                "In",
                repr(self._rmap_header["name"]),
                "applying substitution",
                (keyword, old_parvalue, replacement),
                "transforms",
                repr(old_match),
                "-->",
                repr(new_match),
    ...
    def todict(self):
        ...
        return {
            ...
            "selections": [
                (self.fix_singleton_match_case(key), val.todict())
    ...
    def todict_flat(self):
        ...
        for key, val in self._raw_selections:
            if isinstance(val, Selector):
                ...
                key = tuple([", ".join(glob_list(str(parval))) for parval in self.fix_singleton_match_case(key)])
        ...
        return {"parameters": pars, "selections": flat}
    ...
    def __repr__(self):
        return (
            self.__class__.__name__ + "(" + repr(self._parameters) + ", nselections=" + str(len(self._selections)) + ")"
        )
    ...
    def get_selection(self, lookup_key):
        ...
        """
        raise NotImplementedError("Selector is an abstract class." " Subclasses must re-define get_selection().")
            ...
            for key in self.keys():
                ...
                if len(key) != npars:
                    raise ValidationError(
                        ...
                        + " key="
                        + repr(key)
                        + " is wrong length for parameters "
    ...
    def validate_selector(self, valid_values_map):
        ...
        with log.error_on_exception(self.short_name + repr(self._parameters)):
            if len(self.parkey) != len(self.class_list):
                raise ValidationError(
                    "Length mismatch between rmap parkey:",
                    log.srepr(self.parkey),
                    "and rmap classes header fields",
                    log.srepr(self.class_list),
                )
            ...
            if isinstance(choice, Selector):
                ...
                for val in choice:
                    if not isinstance(val, str):
                        raise ValidationError("Non-string tuple value for choice " + repr(choice) + " at " + repr(key))
            elif isinstance(choice, dict):
                for val in choice:
                    if not isinstance(val, str):
                        raise ValidationError(
                            "Non-string dictionary key for choice " + repr(choice) + " at " + repr(key)
                        )
                for val in choice.values():
                    if not isinstance(val, str):
                        raise ValidationError(
                            "Non-string dictionary value for choice " + repr(choice) + " at " + repr(key)
                        )
            else:
        ...
        for name in self._parameters:
            if name not in header:
                if name in self._parkey_map and "N/A" not in self._parkey_map[name]:
                    raise ValidationError(
                        self.short_name + " required lookup parameter " + repr(name) + " is undefined."
                    )
    ...
    def _validate_value(self, name, value, valid_list, runtime=True):
        ...
        if name in self._substitutions and value in self._substitutions[name]:
            log.verbose(
                ...
                "of",
                repr(value),
                "is substitution from",
                repr(value),
                "to",
                repr(self._substitutions[name]) + ". Checking subsititution value.",
                verbosity=60,
            )
            self._validate_value(name, self._substitutions[name][value], valid_list, runtime)
            return
        raise ValidationError(" parameter=" + repr(name) + " value=" + repr(value) + " is not in " + repr(valid_list))
    ...
    def _validate_raw_key(self, key, valid_values_map):
        ...
        raise NotImplementedError(self.__class__.__name__ + " hasn't defined _validate_raw_key.")
    ...
    def _validate_number(self, parname, value):
        ...
        try:
            ...
            raise ValidationError(self.short_name, "Invalid number for", repr(parname), "value =", repr(value)) from exc
    ...
    def merge(self, other):
        raise AmbiguousMatchError(
            "More than one match was found at the same weight and " + self.short_name + " does not support merging."
        )
    ...
    def difference(self, new_selector, path=(), pars=(), top_selector=True):
        ...
        if self._parameters != new_selector._parameters:
            return [
                msg(None, "different parameter lists ", repr(self._parameters), ":", repr(new_selector._parameters))
            ]
        ...
        for key, choice in self._raw_selections:
            ...
            if key not in new_selector_keys:
                if isinstance(choice, Selector):
                    differences.extend(
                        choice.flat_diff(
                            "deleted {} rule for".format(self.short_name), path + (pkey,), pars + (self._parameters,)
                        )
                    )
                elif top_selector:
                ...
                if isinstance(choice, Selector):
                    differences.extend(
                        choice.difference(
                            new_selector_choice, path + (pkey,), pars + (self._parameters,), top_selector=False
                        )
                    )
        ...
        for key in new_selector_keys:
            ...
            if key not in self_keys:
                ...
                if isinstance(new_selector_choice, Selector):
                    differences.extend(
                        new_selector_choice.flat_diff(
                            "added {} rule for".format(self.short_name), path + (pkey,), pars + (self._parameters,)
                        )
                    )
                elif top_selector:
...
class InequalityMatcher(Matcher):
    ...
    def __init__(self, key):
        ...
        parts = re.match(r"^([><]=?)\s*([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)", key)
...
class BinaryMatcher(Matcher):
    ...
    def match(self, value):
        ...
        if self._operator == "AND":
            return 1 if ((self._matcher1.match(value) == 1) and (self._matcher2.match(value) == 1)) else -1
        elif self._operator == "OR":
            return 1 if ((self._matcher1.match(value) == 1) or (self._matcher2.match(value) == 1)) else -1
        else:
...
def matcher(key):
    ...
    if isinstance(key, tuple):
        ...
        assert float(parts[1]) <= float(parts[2]), (
            "Invalid between relation " + repr(key) + " should be 'between lower_bound upper_bound'"
        )
...
Restore original debug behavior:
        ...
        for i, fitsvar in enumerate(self._parameters):
            ...
            for key in self.keys():
                try:
                    ...
                    raise ValueError(
                        "Match case", repr(key), "wrong length for parameter list", repr(tuple(self._parameters))
                    ) from exc
    ...
    def _check_valid_values(self, valid_values_map):
        ...
        for name in self._parameters:
            if name not in valid_values_map:
                log.verbose_warning(self.short_name, "Parameter ", repr(name), " is unchecked.")
    ...
    def _validate_raw_key(self, key, valid_values_map):
        ...
        if len(key) != len(self._parameters):
            raise ValidationError("wrong length for parameter list " + repr(self._parameters) + " for key " + repr(key))
        for i, name in enumerate(self._parameters):
            ...
            for value in glob_list(str(key[i])):
                log.verbose("Checking", repr(name), "=", repr(value), "against", valid_values_map[name])
                self._validate_value(name, value, valid_values_map[name], runtime=False)
        for other in self.keys():
            if key != other and match_superset(other, key) and not different_match_weight(key, other):
...
Restore debug configuration.
    ...
    def _raw_date(self, header):
        ...
        for par in self._parameters:
            ...
            if hval == "UNDEFINED":
                raise InvalidDatetimeError("UseAfter parameter", repr(par), "is UNDEFINED.")
            date += hval + " "
        return date.strip()
        ...
        while ownsel and othersel:
            ...
            if own.key < other.key:
                ...
                log.verbose(
                    ...
                    repr(appended.key),
                    "using",
                    repr(appended.choice),
                    "not",
                    repr(overwritten.choice),
                    verbosity=10,
                )
...
Restore debug configuration.
    ...
    def _validate_version(self, version):
        assert isinstance(version, str) and config.VERSION_RE.match(version), "Invalid version string " + repr(version)
...
class VersionRelation(ComparableMixin):
        ...
        if self.relation_str.replace("=", "").strip() == "default":
            ...
            if match:
                ...
                try:
                    ...
                    raise ValidationError("Invalid version expression in:", repr(self.relation_str)) from exc
            else:
    ...
    def _check_compatible(self, other):
        if self.version == "default":
            ...
            raise ValidationError(
                "Incompatible version expression types:", str(self.version), "and", str(other.version)
            )
...
def check_duplicates(items, parents=None):
    ...
    for key, value in items:
        if key in already_seen:
            log.error(
                "Duplicate entry at", " ".join(parents + [repr(key)]), "=", repr(value), "vs.", repr(already_seen[key])
            )
        else:
