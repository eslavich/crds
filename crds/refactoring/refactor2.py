...
def rmap_check_modifications(old_rmap, new_rmap, old_ref, new_ref, expected=("add",)):
    ...
    for difference in diffs:
        ...
        if actual in expected:
            ...
            log.error("Expected one of", srepr(expected), "but got", srepr(actual), "from change", srepr(difference))
            as_expected = False
...
def set_rmap_header(rmapping, new_filename, header_key, header_value, *args, **keys):
    ...
    formatting quirks.
    """
    log.verbose("Setting header value in", srepr(rmapping.basename), "for", srepr(header_key), "=", srepr(header_value))
    try:
...
def set_rmap_parkey(rmapping, new_filename, parkey, *args, **keys):
    ...
    references_headers = {
        refname: get_refactoring_header(rmapping.filename, refname, required_keywords) for refname in refnames
    }
...
def set_rmap_substitution(rmapping, new_filename, parameter_name, old_text, new_text, *args, **keys):
    log.info(
        "Adding substitution for",
        srepr(parameter_name),
        "from",
        srepr(old_text),
        "to",
        srepr(new_text),
...
def cat_rmap(rmapping, new_filename, header_key, *args, **keys):
    ...
    if header_key is not None:
        log.info(
            "In", srepr(rmapping.basename), "parameter", srepr(header_key), "=", srepr(rmapping.header[header_key])
        )
    else:
        log.info("-" * 80)
...
def add_rmap_useafter(rmapping, new_filename, *args, **keys):
    ...
    set_rmap_header(rmapping, new_filename, "classes", repr(rmapping.obs_package.DEFAULT_SELECTORS), *args, **keys)
...
def fix_rmap_undefined_useafter(rmapping, new_filename, *args, **keys):
    ...
    if "UNDEFINED UNDEFINED" in str(rmapping):
        rmap.replace_rmap_text(rmapping, new_filename, "UNDEFINED UNDEFINED", "1900-01-01 00:00:00", *args, **keys)
...
def diff_rmap(rmapping, new_filename, *args, **keys):
    ...
    script = diff.DiffScript(
        "crds.diff {0} {1} --brief --mapping-text-diffs "
...
def certify_rmap(rmapping, new_filename, source_context=None, *args, **keys):
    ...
    script = certify.CertifyScript(
        "crds.certify {0} --comparison-context {1}".format(new_filename, rmapping.filename),
        reset_log=False,
        print_status=False,
    )
    script()
...
class RefactorScript2(cmdline.Script):
    ...
    def add_args(self):
        ...
        self.add_argument(
            "--source-context", type=str, default=None, help="Source context from which to retrieve affected mappings."
        )
        ...
        self.add_argument(
            "--inplace", action="store_true", help="Rewrite files directly in cache replacing original copies."
        )
        self.add_argument(
            "--fixers",
            type=str,
            nargs="*",
            default=["FGS1:GUIDER1", "FGS2:GUIDER2"],
        ...
        self.add_argument(
            "--certify-rmaps", action="store_true", help="After refactoring run crds.certify on refactored rmaps."
        )
        self.add_argument(
            "--best-effort",
            action="store_true",
        ...
        with log.error_on_exception("Refactoring operation FAILED"):
            if self.args.command == "insert_reference":
                if self.args.old_rmap:
                    old_rmap, new_rmap = (
                        self.resolve_context(self.args.old_rmap),
                        self.resolve_context(self.args.new_rmap),
                    )
    ...
    def rmap_apply(self, func, *args, **keys):
        ...
        if self.args.rmaps:
            for rmap_name in self.args.rmaps:
                with log.error_on_exception("Failed processing rmap", srepr(rmap_name)):
                    ...
                    self._diff_and_certify(
                        rmapping=rmapping, new_filename=new_filename, source_context=self.source_context, **keywords
                    )
        else:
            ...
            for instr in instruments:
                with log.augment_exception("Failed loading imap for", repr(instr), "from", repr(self.source_context)):
                ...
                for filekind in types:
                    with log.error_on_exception("Failed processing rmap for", repr(filekind)):
                        ...
                        new_filename = self._process_rmap(func, rmapping=rmapping, **keywords)
                        self._diff_and_certify(
                            rmapping=rmapping, source_context=self.source_context, new_filename=new_filename, **keywords
                        )
    ...
    def set_substitution(self):
        """Do simple text substitution in elaborated rmaps replacing `args.old_text` with `args.new_text`."""
        self.rmap_apply(
            set_rmap_substitution,
            parameter_name=self.args.parameter_name,
            old_text=self.args.old_text,
            new_text=self.args.new_text,
        )
